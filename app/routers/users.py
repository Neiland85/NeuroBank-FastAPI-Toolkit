...
async def get_user(
    user_id: str = Path(...), db: AsyncSession = Depends(get_db), current_user=Security(require_permissions(), scopes=["users:read"])
):
...
async def create_user_admin(
    payload: UserCreate, db: AsyncSession = Depends(get_db), current_user=Security(require_permissions(), scopes=["users:write"])
):
...
async def delete_user_endpoint(
    user_id: str, db: AsyncSession = Depends(get_db), current_user=Security(require_permissions(), scopes=["users:delete"])
):
...
async def assign_roles_endpoint(
    user_id: str, role_names: List[str], db: AsyncSession = Depends(get_db), current_user=Security(require_permissions(), scopes=["users:write"])
):
